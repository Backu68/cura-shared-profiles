...
## 0.9.0-alpha.2 refactor regression

These tests are in addition to the existing synchronization/deletion/conflict matrix below.

1. Upgrade an existing 0.8.7 installation that already has a library path and synchronized quality state. Expected: Eventide loads the same path/state from Cura Preferences after one-time migration; the old JSON config file is left in place as a recovery artifact.
2. Restart Cura after migration. Expected: Eventide continues using Cura Preferences and does not rewrite the legacy local JSON config.
3. Leave a connected NAS/share unavailable for at least 30 seconds. Expected: Cura's UI remains responsive while Eventide reports the share unavailable; routine watcher passes do not synchronously crawl the share on the UI thread.
4. Restore the share. Expected: Eventide notices it and converges without restarting Cura.
5. Edit a local custom quality setting. Expected: the change publishes after the event/debounce pass; an idle unedited profile is not republished on each 10-second monitor pass.
6. Repeat the existing offline stale-client and deletion-vs-edit race tests. Expected: receive/deletion handling still wins before dirty local publication and no stale copy silently resurrects a deletion.
7. With Cura's global settings tab selected, verify Eventide follows Cura's default enabled extruder. Select a disabled extruder explicitly if Cura permits it. Expected: Eventide reports that selected stack rather than silently substituting extruder 0.
8. Slice a matching capability with max linear speed / max volumetric flow. Expected: CuraEngine receives the transient capped settings and Eventide does not modify the finished G-code afterward.
...
6. Expected: the modal does not repeat on each background polling cycle for the same conflict state.
